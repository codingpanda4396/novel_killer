...
def continuity_check_node(state: PipelineState) -> dict[str, Any]:
    ...
    try:
        ...
        # 读取连续性上下文
        continuity_context = _read_continuity_context(paths)
...
def _read_continuity_context(paths: ProjectPaths) -> str:
    """读取连续性上下文"""
    parts = []

    # 读取角色状态
    char_state = paths.state / "character_state.md"
    ...
    # 读取时间线
    timeline = paths.state / "timeline.md"
    if timeline.is_file():
    ...
    # 读取活跃线索
    threads = paths.state / "active_threads.md"
    if threads.is_file():
